...
#for each transcript it will output the longest ORF/CDS that can be produced from it
print "usage: perl GetLongestCDS_vX.pl fastafile\n";

#V2: fixed problem with how "N"s were handled

my $fasta = $ARGV[0];
open (FASTAFILE, "$fasta") or die "could not find fasta";
open (RESULTS, ">$fasta.cds");
...
#this returns the longest ORF for a sequence/frame
sub longestaa
{

	my ($dna, @nums, @codons, @orfs, $longestorf);
	$dna=$_[0];
	$dna =~ s/>//g;
...
my $len = length $orfs[0];
my $longest = 0;
for my $i (1 .. $#orfs) {
    my $i_len = length $orfs[$i];
    if($i_len > $len) {
        $longest = $i;
        $len = $i_len;
    }
}
	my $longestorf = $orfs[$longest];
