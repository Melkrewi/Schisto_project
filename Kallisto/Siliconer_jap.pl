...
while ($run = <INPUT>) 
	...
	if (-e $filename)
		...
		if (-e $filename2)
			...
			#if yes
			system "kallisto quant -t 16 -i /nfs/scistore03/vicosgrp/mpicard/NEW_expressionanalysis/kallisto_index_japonicum/transcripts.idx -o KalFiles/$run -b 100 $run\_1.fastq $run\_2.fastq";
			}
		else
			{
			...
			system "kallisto quant -t 16 -i /nfs/scistore03/vicosgrp/mpicard/NEW_expressionanalysis/kallisto_index_japonicum/transcripts.idx -o KalFiles/$run -b 100 --single -l 180 -s 20 $run\_1.fastq";
			}
