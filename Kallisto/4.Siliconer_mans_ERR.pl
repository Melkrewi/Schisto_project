...
### 2. Run Kallisto
        ...
        if (-e $filename2)
            {
            #if yes
            system "kallisto quant -t 16 -i ~/path-to-mansoni-kallisto-output/transcripts.id -o KalFiles/$run -b 100 $run\_1.fastq $run\_2.fastq";
            }
        else
            {
            #if no
            print "$run is single-end\n";
            system "kallisto quant -t 16 -i ~/path-to-mansoni-kallisto-output/transcripts.id -o KalFiles/$run -b 100 --single -l 180 -s 20 $run\_1.fastq";
            }
